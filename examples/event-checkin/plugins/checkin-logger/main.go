...
func main() {
	...
	http.HandleFunc("/execute", func(w http.ResponseWriter, r *http.Request) {
		...
		attendee, _ := payload["attendee_name"].(string)
		log.Printf("[checkin-logger] tenant=%s effect=%s registration=%s event=%q attendee=%q transition=%s",
	...
	log.Printf("checkin-logger plugin listening on :%s", port)
	if err := server.ListenAndServe(); err != nil {
		log.Fatalf("checkin-logger error: %v", err)
	}
}
