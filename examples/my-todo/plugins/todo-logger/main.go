...
const effectName = "my_todo_log_task_event"
...
func main() {
	...
	http.HandleFunc("/health", func(w http.ResponseWriter, r *http.Request) {
		...
		json.NewEncoder(w).Encode(map[string]any{
			"status":            "ok",
			"plugin":            "todo-logger",
			"supported_effects": []string{effectName},
		})
	})
	http.HandleFunc("/execute", func(w http.ResponseWriter, r *http.Request) {
		...
		assigneeUserID, _ := payload["assignee_user_id"].(string)
		log.Printf("[todo-logger] tenant=%s task=%s title=%q transition=%s assignee_user_id=%s",
			req.TenantID, req.EntityID, taskTitle, req.Transition, assigneeUserID)
		writeSuccess(w)
	})
	server := &http.Server{
		Addr:         ":" + port,
	...
	log.Printf("todo-logger plugin listening on :%s", port)
	if err := server.ListenAndServe(); err != nil {
		log.Fatalf("todo-logger error: %v", err)
	}
}

func writeSuccess(w http.ResponseWriter) {
	w.Header().Set("Content-Type", "application/json")
	json.NewEncoder(w).Encode(effectResponse{Success: true})
}

func writeError(w http.ResponseWriter, status int, message string, shouldRetry bool) {
	w.Header().Set("Content-Type", "application/json")
	w.WriteHeader(status)
	json.NewEncoder(w).Encode(effectResponse{Success: false, ErrorMessage: message, ShouldRetry: shouldRetry})
}
